...
\vspace{2ex}
\begin{center}
{\underline{\Large \bf Gradient Descent}}
\end{center}
\begin{abstract}
This document contains the solution to find the maximum / minimum value of given function by gradient descent method
\end{abstract}
Download all python codes from 
%
\begin{lstlisting}
https://github.com/rubeenaafreen20/EE5600AI-ML/tree/master/Assignment4/Code
\end{lstlisting}
%
Download latex-tikz codes from 
%
\begin{lstlisting}
https://github.com/rubeenaafreen20/EE5600AI-ML/tree/master/Assignment4
\end{lstlisting}
%
\section{Problem}
Find the maximum and minimum values, if any, of the follwing function
\begin{align}
    f(x)=9x^2+12x+2 \label{eq:1}
\end{align}
...
&9\lambda(1-\lambda)(x_1-x_2)^2 \geq 0 \label{eq:2}
\end{align}
Equation \eqref{eq:2} holds true for all $\lambda\in(0,1)$. Hence the given function f(x) is convex.
\newline Fora general quadratic equation
\begin{align}
    f(x)=ax^2+bx+c
\end{align}
The update equation for gradient descent to find minimum of a function is given by:
\begin{align}
    \lambda_{n+1}=\lambda_n-\mu f'(\lambda_n) \label{eq:2}\\
    =\lambda_n-\mu (2a\lambda_n +b)
\end{align}
In equation \eqref{eq:2} $\lambda_0$ is an initial guess and $\mu$ is a variable parameter,known as step size $\lambda_{n+1}$ is the next position. The minus sign refers to the minimization part of gradient descent.
\newline Assume,
\begin{align}
    \lambda_0=1\\
    \mu=0.001 \\
...
\end{enumerate}
Figure \ref{Fig:1} shows plot of parabola obtained from python code:\newpage
\begin{figure}[h]
